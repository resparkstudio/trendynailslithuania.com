...
$product_price = $main_product->get_price_html();
$product_short_description = $main_product->get_short_description();
$product_long_description = $main_product->get_description();
$product_image_id = $main_product->get_image_id();
$product_gallery_image_ids = $main_product->get_gallery_image_ids();
$product_sku = $main_product->get_sku();
$product_category = wc_get_product_category_list($product_id);
$product_weight = $main_product->get_weight();
$product_attributes = $main_product->get_attributes();
?>


<section id="primary">
	<main id="main" class="max-w-[87.5rem] mx-auto w-full">
		<div id="page-content" class="mx-12 md:mx-4 flex justify-center align-center flex-wrap mt-9 mb-40">
			<div id="product-section" class="grid grid-cols-12 gap-4">
				...
				<div class="col-span-6">
					...
					<p class="text-lg"><?php echo $main_product->get_attribute('size'); ?> ML</p>
			...
			<!-- Similar products -->
			<?php
			$current_product = wc_get_product(); // Get current product
			$current_category_ids = wp_get_post_terms($current_product->get_id(), 'product_cat', array('fields' => 'ids'));
			...
			$args = array(
				...
				'tax_query' => array(
					array(
						...
						'field' => 'term_id',
						'terms' => $current_category_ids, // Filter by the current product's categories
						'operator' => 'IN',
					),
				),
				'post__not_in' => array($current_product->get_id()), // Exclude the current product
			);
			$similar_product_loop = new WP_Query($args);
			?>
